/* Pages légales (mentions légales, confidentialité, cookies), affichées en fenêtre modale
   depuis le pied de page — même contenu que le footer du site principal (lameutenormande.fr),
   la base de données des sondages étant la même (schéma "votes", partagé avec la page Actus). */
(function () {
  'use strict';
  const TEXTS = {
    mentions: {
...
<p><em>Dernière mise à jour : septembre 2026</em></p>`
  ...
  window.V = window.V || {};
  V.LEGAL = TEXTS;
  V.openLegal = function (key) {
    const t = TEXTS[key]; if (!t) return;
    const close = () => { bg.remove(); document.removeEventListener('keydown', onKey); };
    const onKey = (e) => { if (e.key === 'Escape') close(); };
    const bg = V.h('div', { class: 'legal-modal-bg', onClick: (e) => { if (e.target === bg) close(); } },
      V.h('div', { class: 'legal-modal', role: 'dialog', 'aria-modal': 'true', 'aria-label': t.title },
        V.h('div', { class: 'legal-modal-header' }, V.h('h3', { class: 'legal-modal-title' }, t.title),
          V.h('button', { class: 'legal-modal-close', type: 'button', 'aria-label': 'Fermer', onClick: close }, '×')),
        V.h('div', { class: 'legal-modal-body', html: t.html })));
