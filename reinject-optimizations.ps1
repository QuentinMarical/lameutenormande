...
# Ajouter le CSS du système de calendrier pour la page événements
$eventsCalendarCSS = '  <link rel="stylesheet" href="assets/css/events-calendar.css">'
Insert-BeforeTag -filePath "$projectPath\événements.html" -searchTag "</head>" -contentToInsert $eventsCalendarCSS

$mobileMenuFixJS = '<script src="assets/js/mobile-menu-fix.js"></script>'
Insert-BeforeTag -filePath "$projectPath\index.html" -searchTag "<script src=`"assets/js/init.js`"></script>" -contentToInsert $mobileMenuFixJS
Insert-BeforeTag -filePath "$projectPath\événements.html" -searchTag "<script src=`"assets/js/init.js`"></script>" -contentToInsert $mobileMenuFixJS
...
$googleMapsAPI = '<script src="https://maps.googleapis.com/maps/api/js?key=YOUR_API_KEY&callback=Function.prototype" async defer></script>'
Insert-BeforeTag -filePath "$projectPath\événements.html" -searchTag "</body>" -contentToInsert $googleMapsAPI

# Ajouter le JS du système de calendrier pour la page événements
$eventsCalendarJS = '<script src="assets/js/events-calendar.js"></script>'
Insert-BeforeTag -filePath "$projectPath\événements.html" -searchTag "</body>" -contentToInsert $eventsCalendarJS

# Ajouter le JS de correction du menu mobile
$mobileMenuFixJS = '<script src="assets/js/mobile-menu-fix.js"></script>'
Insert-BeforeTag -filePath "$projectPath\événements.html" -searchTag "</body>" -contentToInsert $mobileMenuFixJS
...
Write-Host "  • Système calendrier interactif installé" -ForegroundColor White
...
Write-Host ""
Write-Host "??  IMPORTANT: N'oubliez pas de remplacer YOUR_API_KEY" -ForegroundColor Yellow
Write-Host "   par votre vraie clé Google Maps API !" -ForegroundColor Yellow
Write-Host ""
